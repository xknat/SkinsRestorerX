...
public class SkinCommand extends BaseCommand {
    ...
    public void onSkinSetShort(ProxiedPlayer p, String skin) {
        this.onSkinSetOther(p, new OnlinePlayer(p), skin, null);
    }
    ...
    public void onSkinClearOther(CommandSender sender, @Single OnlinePlayer target) {
        ProxyServer.getInstance().getScheduler().runAsync(SkinsRestorer.getInstance(), () -> {
            ...
            if (this.setSkin(sender, p, skin, false, true, null)) {
                if (sender == p)
    ...
    public void onSkinUpdateOther(CommandSender sender, @Single OnlinePlayer target) {
        ProxyServer.getInstance().getScheduler().runAsync(SkinsRestorer.getInstance(), () -> {
            ...
            if (this.setSkin(sender, p, skin, false, false, null)) {
                if (sender == p)
    ...
    public void onSkinSet(ProxiedPlayer p, String[] skin) {
        if (skin.length > 0) {
            this.onSkinSetOther(p, new OnlinePlayer(p), skin[0], null);
        } else {
    ...
    public void onSkinSetOther(CommandSender sender, OnlinePlayer target, String skin, @Optional SkinType skinType) {
        ProxyServer.getInstance().getScheduler().runAsync(SkinsRestorer.getInstance(), () -> {
            ...
            if (this.setSkin(sender, p, skin, true, false, skinType) && (sender != p)) {
    ...
    public void onSkinSetUrl(ProxiedPlayer p, String url, @Optional SkinType skinType) {
        if (C.validUrl(url)) {
            this.onSkinSetOther(p, new OnlinePlayer(p), url, skinType);
        } else {
    ...
    private boolean setSkin(CommandSender sender, ProxiedPlayer p, String skin) {
        return this.setSkin(sender, p, skin, true, false, null);
    }
